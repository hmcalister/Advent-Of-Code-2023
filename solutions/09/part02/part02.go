...
func (history historyData) findPreviousValueInSequence() int {
	var currentSequence []int
	var nextSequence []int
	var nextSequenceAllZero bool
	firstItemInSequences := make([]int, 0)

	currentSequence = make([]int, len(history.HistoricValues))
	copy(currentSequence, history.HistoricValues)
	// slices.Reverse(currentSequence)
	...
	for !nextSequenceAllZero {
		...
		firstItemInSequences = append(firstItemInSequences, currentSequence[0])
		currentSequence = nextSequence
	}

	cumulativeTerm := 0
	slices.Reverse(firstItemInSequences)
	log.Trace().
		Interface("FirstItemInSequences", firstItemInSequences).
		Send()
	for _, s := range firstItemInSequences {
		log.Trace().
			Int("CurrentCumulativeTerm", cumulativeTerm).
			Int("NextFirstItem", s).
			Int("NextTerm", s-cumulativeTerm).
			Send()
		cumulativeTerm = s - cumulativeTerm
	}

	return cumulativeTerm
}
...
func ProcessInput(fileScanner *bufio.Scanner) (int, error) {
	...
	for fileScanner.Scan() {
		...
		result += history.findPreviousValueInSequence()
	}
